...
class TestResourceWorldInitialize:
    def test_world_state_has_correct_capacities(self):
        world = ResourceWorld(
            ...
            resource_capacities=[2, 3],
        )
        state, dynamics = world.initialize(agent_ids=[0, 1, 2, 3], rng=_make_rng())
...
class TestResourceWorldFilterActions:
    def setup_method(self):
        self.world = ResourceWorld(
            collective_size_distribution={2: 1.0},
            resource_capacities=[1],
        )  # one resource, cap 1

    def _state(self):
        state, _ = self.world.initialize(agent_ids=[0, 1], rng=_make_rng())
        return state
    ...
    def test_feasible_actions_returned(self):
        ...
        actions = {
            0: _resource_action(0, 1, bid=2),
            1: _resource_action(0, 1, bid=1),
        }
        ca = self.world.filter_actions(state, agent_weights={0: 1, 1: 1}, agent_actions=actions)
        assert len(ca.decisions) > 0

    def test_no_more_than_capacity_agents_assigned(self):
        state = self._state()
        actions = {
            0: _resource_action(0, 1, bid=1),
            1: _resource_action(0, 1, bid=1),
        }
        ca = self.world.filter_actions(state, agent_weights={0: 1, 1: 1}, agent_actions=actions)
    ...
    def test_agents_can_request_null_outcome(self):
        state = self._state()
        actions = {0: _null_action(1), 1: _null_action(1)}
        ca = self.world.filter_actions(state, agent_weights={0: 1, 1: 1}, agent_actions=actions)
    ...
    def test_signals_shape_matches_decisions(self):
        state = self._state()
        actions = {0: _resource_action(0, 1, bid=3), 1: _null_action(1)}
        ca = self.world.filter_actions(state, agent_weights={0: 1, 1: 1}, agent_actions=actions)
        n_agents = 2
    ...
    def test_mismatched_ids_raises(self):
        state = self._state()
        actions = {0: _null_action(1)}
        with pytest.raises(ValueError):
            self.world.filter_actions(state, agent_weights={0: 1, 1: 1}, agent_actions=actions)  # 2 ids, 1 action


class TestResourceWorldMultiResource:
    def setup_method(self):
        self.world = ResourceWorld(
            collective_size_distribution={3: 1.0},
            resource_capacities=[1, 1],
        )

    def test_feasibility_respected_per_resource(self):
        state, _ = self.world.initialize(agent_ids=[0, 1, 2], rng=_make_rng())
        n = 2
        actions = {
            0: _resource_action(0, n, bid=2),
            1: _resource_action(1, n, bid=2),
            2: _resource_action(0, n, bid=1),
        }
        ca = self.world.filter_actions(state, agent_weights={0: 1, 1: 1, 2: 1}, agent_actions=actions)
        for decision in ca.decisions:
    ...
    def test_update_state_is_identity(self):
        state, _ = self.world.initialize(agent_ids=[0, 1], rng=_make_rng())
        new_state = self.world.update_state(state, reports={}, rng=_make_rng())
