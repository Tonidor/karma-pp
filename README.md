...
kpp run --scenario data/scenarios/2x2_q_learners.yaml --steps 500 --seed 7 --log-level DEBUG
...
# Scenarios must be passed in order: random, optimal, dictator, q-learner
...
# Scenarios must be passed in order: random, q-learner, optimal
...
```
data/
  scenarios/    # Experiment entry points (YAML)
  worlds/       # World configuration (YAML)
  mechanisms/   # Mechanism configuration (YAML)
src/karma_pp/
...
1. Create a scenario YAML in `data/scenarios/`. Reference world and mechanism files using paths **relative to the scenario file** (e.g. `../worlds/my_world.yaml`).
2. Create or reuse world/mechanism YAMLs in `data/worlds/` and `data/mechanisms/`.
3. Run with `kpp run --scenario data/scenarios/my_scenario.yaml`.
